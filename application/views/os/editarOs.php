            ...
            <div class="widget-content nopadding tab-content">
                <div class="span12" id="divProdutosServicos" style=" margin-left: 0">
                    <ul class="nav nav-tabs">
                        ...
                        <li id="tabCarros"><a href="#tab6" data-toggle="tab">Carros</a></li>
                    </ul>
                    <div class="tab-content">
                        ...
                        <div class="tab-pane" id="tab6">
                            <div class="span12 well" style="padding: 1%; margin-left: 0">
                                <?php $disabled = $carros != null ? "disabled='disabled'" : ""; ?>
                                <form id="formCarros" action="<?php echo base_url() ?>index.php/os/adicionarCarro" method="post">
                                    <div class="span6">
                                        <input type="hidden" name="idCarro" id="idCarro" />
                                        <input type="hidden" name="idOsServico" id="idOsServico" value="<?php echo $result->idOs; ?>" />
                                        <label for="">Carro</label>
                                        <input type="text" class="span12" name="carro" id="carro" placeholder="Digite o carro ou placa" />
                                    </div>
                                    <div class="span2">
                                        <label for="">&nbsp;</label>
                                        <!-- <button class="btn btn-success span12" id="btnAdicionarCarro"><i class="fas fa-plus"></i> Adicionar -->
                                        <button class="btn btn-success span12" id="btnAdicionarCarro" <?php echo $disabled; ?>><i class="fas fa-plus"></i> Adicionar
                                        </button>
                                    </div>
                                </form>
                            </div>
                            <div class="widget-box" id="divCarros">
                                <div class="widget_content nopadding">
                                    <table width="100%" class="table table-bordered" id="tblCarros">
                                        <thead>
                                            <tr>
                                                <th>Carro</th>
                                        ...
                                        <tbody>
                                            ...
                                            foreach ($carros as $p) {
                                                echo '<tr>';
                                                echo '<td>' . $p->carroSimples . '</td>';
                                                echo '<td><div align="center">' . $p->placa . '</td>';
                                                echo '<td><div align="center">' . $p->montadora  . '</td>';
                                                echo '<td><div align="center"><span idAcao="' . $p->idCarros_os . '" title="Excluir Carro" class="btn btn-danger carro"><i class="fas fa-trash-alt"></i></span></div></td>';
                                                echo '</tr>';
                                            } ?>
                                        </tbody>
                                    </table>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#carro").autocomplete({
            source: "<?php echo base_url(); ?>index.php/os/autoCompleteCarro",
            minLength: 2,
            select: function(event, ui) {
                $("#idCarro").val(ui.item.id);
                $("#carro").val(ui.item.carro);
                //$("#quantidade_servico").focus();
            }
        });
        ...
        $("#formCarros").validate({
            ...
            messages: {
                carro: {
                    required: 'Insira um carro'
                },
            },
            submitHandler: function(form) {
                var dados = $(form).serialize();

                $("#divCarros").html("<div class='progress progress-info progress-striped active'><div class='bar' style='width: 100%'></div></div>");
                $.ajax({
                    type: "POST",
                    ...
                    success: function(data) {
                        if (data.result == true) {
                            $("#divCarros").load("<?php echo current_url(); ?> #divCarros");
                            $("#carro").val('');
                            $("#btnAdicionarCarro").attr('disabled', 'disabled');
                        } else {
                            Swal.fire({
                                type: "error",
                                title: "Atenção",
        ...
        $(document).on('click', '.carro', function(event) {
            var idCarro = $(this).attr('idAcao');
            var idOS = "<?php echo $result->idOs ?>"
            if ((idCarro % 1) == 0) {
                $("#divCarros").html("<div class='progress progress-info progress-striped active'><div class='bar' style='width: 100%'></div></div>");
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url(); ?>index.php/os/excluirCarro",
                    data: "idCarro=" + idCarro + "&idOs=" + idOS,
                    //data: "idCarro=" + idCarro,
                    dataType: 'json',
                    success: function(data) {
                        if (data.result == true) {
                            $("#divCarros").load("<?php echo current_url(); ?> #divCarros");
                            $("#btnAdicionarCarro").removeAttr('disabled');
                        } else {
                            Swal.fire({
                                type: "error",
                                title: "Atenção",
                                text: "Ocorreu um erro ao tentar excluir carro."
                            });
                        }
                    }
                });
