...
class Os_model extends CI_Model
{
    ...
    {
        $this->db->select('carros_os.*, carros.carro as carroSimples, carros.placa, carros.montadora');
        $this->db->from('carros_os');
        $this->db->join('carros', 'carros.idCarros = carros_os.carros_id');
        $this->db->where('os_id', $id);
    ...
    public function autoCompleteCarro($q, $idCliente)
    {
        $this->db->select('*');
        $this->db->limit(5);
        $this->db->where('idClientes', $idCliente);
        $this->db->like('carro', $q);
        $this->db->or_like('placa', $q);
        $query = $this->db->get('carros');
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $row_set[] = ['label' => $row['carro'] . ' | Placa: ' . $row['placa'] . ' | Montadora: ' . $row['montadora'], 'id' => $row['idCarros'], 'carro' => $row['carro'], 'montadora' => $row['montadora']];
            }
            echo json_encode($row_set);
        }
    }
