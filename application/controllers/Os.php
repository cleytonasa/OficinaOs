...
class Os extends MY_Controller
{
    ...
    public function autoCompleteCarro()
    {
        if (isset($_GET['term'])) {
            $q = strtolower($_GET['term']);
            //$idCliente = $data;
            $idCliente = $this->session->userdata('clientes');
            $this->os_model->autoCompleteCarro($q, $idCliente['idClientes']);
        }
    }
    ...
    public function excluirCarro()
    {
        ...
        $os = $this->os_model->getById($idOs);
        if ($os == null) {
            $this->session->set_flashdata('error', 'Erro ao tentar excluir carro na OS.');
            redirect(base_url() . 'index.php/os/gerenciar/');
        }

        if ($this->os_model->delete('carros_os', 'idCarros_os', $id) == true) {
            // $quantidade = $this->input->post('quantidade');
            // $produto = $this->input->post('produto');
            ...
            //     $this->produtos_model->updateEstoque($produto, $quantidade, '+');
            // }
            log_info('Removeu carro de uma OS. ID (OS): ' . $idOs);
    ...
    public function adicionarCarro()
    {
        ...
        // if ($this->form_validation->run('adicionar_carro_os') === false) {
        //     $errors = validation_errors();
